...
function setListeners(userCard) {
    ...
    changeBtn.addEventListener('click', () => {
        ...
        const newHobby = document.querySelector('#hobby')

        newName.value = userCard.querySelectorAll('p')[0].innerText
        newSecondName.value = userCard.querySelectorAll('p')[1].innerText
        newEmail.value = userCard.dataset.email
        newHobby.value = userCard.querySelectorAll('p')[3].innerText
    })
}
 ...
 */
function createCard({ name, secondName, email, hobby }) {
    return `
...
function addCard(e) {
    ...
    const users = document.querySelector('.users')
    const usersEmail = document.querySelector(
    ...
    if (usersEmail === null) {
        if (
            ...
            resetInputs(newName, newSecondName, newEmail, newHobby)
            return
        }
        ...
        if (newEmail.value === usersEmail.dataset.email) {
            usersEmail.querySelectorAll('p')[0].innerText = newName.value
            usersEmail.querySelectorAll('p')[1].innerText = newSecondName.value
            usersEmail.querySelectorAll('p')[3].innerText = newHobby.value

            storage[usersEmail.dataset.email].name = newName.value
            storage[usersEmail.dataset.email].secondName = newSecondName.value
            storage[usersEmail.dataset.email].hobby = newHobby.value
            localStorage.setItem('users', JSON.stringify(storage))
